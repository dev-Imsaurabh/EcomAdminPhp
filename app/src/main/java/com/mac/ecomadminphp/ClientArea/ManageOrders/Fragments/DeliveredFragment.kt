...
import com.mac.ecomadminphp.databinding.FragmentDeliveredBinding
...
class DeliveredFragment : Fragment() {
    ...
    private var deliveredList = mutableListOf<CommonOrderModel>()
                ...
                try {
                    ...
                    if(success.equals("1")){
                        for(item in 0 until jsonArray.length()){
                            ...
                            val data = CommonOrderModel(id,orderId,total,status,orderManagerId,paymentMode,"2",dc)
                            if(data.status.equals("1")){
                                deliveredList.add(0,data)
                            }

                        }

                        binding.deliverRecycler.setHasFixedSize(true)
                        binding.deliverRecycler.layoutManager= LinearLayoutManager(context)
                        val adapter = context?.let { CommonOrderAdapter(it,deliveredList) }
                        if (adapter != null) {
                            adapter.notifyDataSetChanged()
                        }
                        binding.deliverRecycler.adapter=adapter
